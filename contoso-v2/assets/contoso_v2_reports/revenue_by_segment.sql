...
name: contoso_v2_reports.revenue_by_segment
type: bq.sql
description: |
...
columns:
  ...
  - name: units_sold
    type: INTEGER
    description: |
      Net units sold for the segment — SUM(quantity), where positive line items are sales
      and negative line items are returns. In tiny segments with few orders, returns can
      flip the net to zero or negative. Essential for inventory planning and demand forecasting.
    checks:
      - name: not_null
  - name: revenue_usd
    type: DOUBLE
  ...
  - name: avg_order_value
    type: DOUBLE
    description: |
      Average order value in US Dollars (revenue / distinct order count). Can be zero or
      negative for tiny segments dominated by returns / clearance line items. Key customer
      behavior metric for basket size and pricing strategy.
    checks:
      - name: not_null
  - name: revenue_mom
    type: DOUBLE
...
WITH monthly AS (
    ...
    FROM contoso_v2_staging.sales_fact
    GROUP BY 1, 2, 3, 4
)
