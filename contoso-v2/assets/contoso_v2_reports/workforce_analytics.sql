...
name: contoso_v2_reports.workforce_analytics
type: bq.sql
description: |
...
depends:
  - contoso_v2_staging.employee_directory
...
headcount AS (
    SELECT
        ...
        ROUND(AVG(CASE
            WHEN e.is_active = TRUE
              OR (e.termination_date IS NOT NULL
                  AND e.termination_date > DATE(q.year, q.quarter * 3, 1))
            THEN e.tenure_months
        END), 1) AS avg_tenure_months,
        ROUND(AVG(CASE
            WHEN e.is_active = TRUE
              OR (e.termination_date IS NOT NULL
                  AND e.termination_date > DATE(q.year, q.quarter * 3, 1))
        ...
        END), 2) AS avg_salary
    FROM quarters q
    CROSS JOIN contoso_v2_staging.employee_directory e
