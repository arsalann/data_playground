...
sprint_metrics AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE status IN ('Done', 'Closed')) AS completed_tickets,
        COUNT(*) FILTER (WHERE ticket_type = 'Bug') AS bug_count,
        COUNT(*) FILTER (WHERE ticket_type = 'Story') AS story_count,
        ...
        ROUND(AVG(
            CASE WHEN resolved_date IS NOT NULL
                THEN DATE_DIFF('day', CAST(created_date AS DATE), CAST(resolved_date AS DATE))
            END
...
deploy_by_week AS (
    SELECT
        CONCAT('Sprint ', EXTRACT(isoyear FROM deploy_date), '-W',
            LPAD(CAST(EXTRACT(week FROM deploy_date) AS VARCHAR), 2, '0')) AS sprint_name,
        COUNT(*) AS deployment_count,
        COUNT(*) FILTER (WHERE status = 'Rolled Back') AS rollback_count
...
SELECT
    ...
    ROUND((dw.rollback_count) / (NULLIF(dw.deployment_count, 0)) * 100, 2) AS rollback_rate
