...
Use the Bruin Cloud API token from the repo-root `.bruin.yml` `bruin` connection named `bruin-cloud` as a project convention. The connection shape is:
...
Useful Cloud CLI commands verified against `/Users/bear/Github/bruin/cmd/cloud.go` and the Bruin Cloud command docs:
...
bruin cloud instances list --project-id <project-id> --pipeline <pipeline> --latest --output json
bruin cloud instances get --project-id <project-id> --pipeline <pipeline> --run-id <run-id> --asset <asset> --output json
bruin cloud instances get --project-id <project-id> --pipeline <pipeline> --latest --asset <asset> --output json
bruin cloud instances logs --project-id <project-id> --pipeline <pipeline> --run-id <run-id> --asset <asset> --step-name <step> --output json
bruin cloud instances logs --project-id <project-id> --pipeline <pipeline> --run-id <run-id> --step-id <step-id> --output json
bruin cloud instances failed-logs --project-id <project-id> --pipeline <pipeline> --run-id <run-id> --output json
bruin cloud instances failed-logs --project-id <project-id> --pipeline <pipeline> --latest --output json
```

`bruin cloud pipelines errors --output json` has no `--project-id` flag in the verified source; filter project and pipeline matches client-side after retrieval. Cloud run statuses are `success`, `failed`, and `running`; asset instances may also surface states such as `checks_failed`. `runs trigger` and `runs rerun` return a success envelope, not the new run ID. When a skill needs the resulting run, poll `bruin cloud runs list --project-id <project-id> --pipeline <pipeline> --limit 1 --output json`, then verify the selected run with `bruin cloud runs get --project-id <project-id> --pipeline <pipeline> --run-id <run-id> --output json`.
...
## Local Bruin Diagnostics

Use `bruin query` for read-only warehouse diagnostics. Include an audit description, cap exploratory samples, and dry-run expensive BigQuery scans before execution:

```shell
bruin query --connection <connection> --query <sql> --description "<why this query is being run>" --limit 1000 --output json
bruin query --asset <asset-file> --query <sql> --description "<why this query is being run>" --limit 1000 --output json
bruin query --connection <connection> --query <sql> --description "<why this query is being dry-run>" --dry-run --output json
```

Do not use `--dangerously-bypass-soft-limits` without explicit human approval. `--dry-run` cannot be combined with `--limit` or `--export`.

Local static commands are allowed for evidence and PR validation:

```shell
bruin validate <pipeline-dir-or-asset-file> --output json
bruin lineage <asset-file-path> --output json --full
bruin run --only checks <asset-file>
bruin run --only checks --single-check <check-id> <asset-file>
```

`--single-check` takes a Bruin check ID and only works for one asset path, not a selector or multiple positional assets. When a pipeline uses variants, record the concrete variant from Cloud or finding context and pass it to local static commands, for example `bruin validate <path> --variant <variant> --output json` and `bruin lineage <asset-file-path> --variant <variant> --output json --full`.

Quality check `blocking` defaults to `true` at runtime for both custom checks and column checks. The reviewed asset definition schema table is inconsistent for custom checks; follow the quality docs and source behavior until upstream docs are corrected.

Bruin schedules may use documented values such as `@daily`, `@hourly`, or cron; source validation also accepts `daily`, `hourly`, `weekly`, `monthly`, and continuous forms. Treat `meta.freshness` as a project convention only, not an official Bruin freshness SLA field, unless a future Bruin source/API field is added.
