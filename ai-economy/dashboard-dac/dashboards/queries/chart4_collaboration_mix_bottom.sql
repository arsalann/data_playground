-- Chart 4 (bottom half): occupation groups ranked 11-N by AEI conversation
-- volume. Companion to chart4_collaboration_mix_top.sql so all 20 groups
-- with non-NULL collab data render across two stacked charts.
WITH soc_names AS (
    SELECT * FROM UNNEST([
...
ranked AS (
    SELECT
        *,
        ROW_NUMBER() OVER (ORDER BY usage_count_total DESC) AS rk
    FROM normalised
)
