...
name: contoso_v2_staging.sales_fact
type: bq.sql
description: |
...
depends:
  - contoso_v2_raw.sales
  - contoso_v2_raw.customers
  - contoso_v2_raw.products
  - contoso_v2_raw.stores
  - contoso_v2_raw.dates
...
columns:
  ...
  - name: quantity
    type: INTEGER
    description: |
      Number of units on this line item. Positive for sales (~98.5%); negative for
      return / clawback line items (~1.5%) which mirror an original sale. Use SUM(quantity)
      to get net units after returns.
    checks:
      - name: not_null
  - name: unit_price_usd
    type: DOUBLE
  ...
  - name: currency_code
    type: VARCHAR
    description: |
      Original transaction currency code (ISO 4217). Determined by store country —
      USD, EUR, GBP, CAD, AUD historically, plus SEK once the Online SE store opens
      in 2021. Preserved for audit trails and exchange rate impact analysis.
    checks:
      - name: not_null
      - name: accepted_values
        value:
