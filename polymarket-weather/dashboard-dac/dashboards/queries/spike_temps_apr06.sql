...
-- 2026-04-06 (event day only — 24 rows). The Polymarket Yes-price for the
-- 21 °C bucket is returned alongside as `yes_price` (0–1 implied probability)
-- and rendered on the chart's RIGHT y-axis via `yRight: [yes_price]` in YAML.
-- The dual y-axis is a fork-local extension to DAC's line chart; see DAC.md.
WITH temps AS (
  SELECT
    ts_local_paris,
  ...
  WHERE event_local_date = DATE '2026-04-06'
    AND DATE(ts_local_paris) = DATE '2026-04-06'
...
winning AS (
  ...
  WHERE r.event_local_date = DATE '2026-04-06'
    AND m.bucket_kind = 'point'
    AND m.series_slug = 'paris-daily-weather'
),
prices_hourly AS (
  -- Latest Yes-price tick within each event-day hour. Multiplied by 25 so
  -- a 0–1 implied probability sits cleanly above the 0–24 °C temp range.
  SELECT
  ...
  WHERE p.outcome_label = 'Yes'
    AND DATE(p.ts_local_paris) = DATE '2026-04-06'
  GROUP BY ts_hour
)
SELECT
