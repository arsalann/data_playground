...
name: contoso_v2_staging.financial_summary_monthly
type: bq.sql
description: |
...
depends:
  - contoso_v2_raw.gl_journal_entries
  - contoso_v2_raw.budgets
  - contoso_v2_raw.departments
...
columns:
  ...
  - name: department_key
    type: INTEGER
    description: Department identifier (1-12). Primary key component referencing contoso_v2_raw.departments. Maps to business units like Sales, Finance, Marketing.
    primary_key: true
    nullable: false
  - name: account_code
    type: VARCHAR
...
WITH gl_deduped AS (
    SELECT *
    FROM contoso_v2_raw.gl_journal_entries
...
departments_deduped AS (
    SELECT *
    FROM contoso_v2_raw.departments
