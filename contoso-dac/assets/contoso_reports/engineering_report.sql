...
SELECT
    ...
    ROUND((SUM(completed_tickets)) / (NULLIF(SUM(total_tickets), 0)) * 100, 2) AS completion_rate,
    SUM(total_story_points) AS total_story_points,
    SUM(completed_story_points) AS completed_story_points,
    ROUND((SUM(completed_story_points)) / (NULLIF(COUNT(*), 0)), 2) AS avg_velocity,
    SUM(bug_count) AS total_bugs,
    ROUND((SUM(bug_count)) / (NULLIF(SUM(total_tickets), 0)) * 100, 2) AS bug_ratio,
    ...
    ROUND((SUM(deployment_count)) / (NULLIF(COUNT(*), 0)), 2) AS deploy_frequency
