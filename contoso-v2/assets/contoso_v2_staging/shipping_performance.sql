...
name: contoso_v2_staging.shipping_performance
type: bq.sql
description: |
...
depends:
  - contoso_v2_raw.shipments
  - contoso_v2_raw.orders
  - contoso_v2_raw.stores
...
columns:
  ...
  - name: delivery_date
    type: DATE
    description: Actual date customer received the shipment (NULL for In Transit / Lost / Returned).
  - name: carrier
    type: VARCHAR
    description: Shipping carrier name. Carrier mix evolves over time (Amazon Logistics emerges 2018+).
    checks:
      - name: not_null
  ...
  - name: shipment_status
    type: VARCHAR
    description: Final shipment outcome. ~88% Delivered baseline; In Transit / Returned / Lost cover the rest.
    checks:
      - name: not_null
      - name: accepted_values
        value:
  ...
  - name: ship_cost
    type: DOUBLE
    description: Shipping cost in USD - ranges $5-45 based on realistic fulfillment pricing.
    checks:
      - name: not_null
      - name: positive
  - name: store_country
  ...
  - name: days_in_transit
    type: INTEGER
    description: Transit time metric - days from dispatch to delivery (NULL for non-delivered shipments).
  - name: total_lead_days
    type: INTEGER
    description: End-to-end fulfillment time - total days from order to delivery (NULL for non-delivered shipments).
  - name: year
    type: INTEGER
