...
SELECT
    ...
    COUNT(*) FILTER (WHERE shipment_status = 'Returned') AS returned_count,
    COUNT(*) FILTER (WHERE shipment_status = 'Lost') AS lost_count,
    ROUND(COUNT(*) FILTER (WHERE shipment_status = 'Delivered') / COUNT(*) * 100, 2) AS delivery_rate,
    ...
    CAST(QUANTILE_CONT(total_lead_days, 0.95) AS INTEGER) AS p95_lead_days,
