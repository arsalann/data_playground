...
SELECT
    ...
    DATE_DIFF('day', CAST(c.start_date AS DATE), CAST(c.end_date AS DATE)) AS duration_days,
    c.budget_amount,
    ...
    ROUND((a.total_clicks) / (NULLIF(a.total_impressions, 0)) * 100, 2) AS ctr,
    ROUND((a.total_conversions) / (NULLIF(a.total_clicks, 0)) * 100, 2) AS conversion_rate,
    COALESCE(r.attributed_orders, 0) AS attributed_orders,
    ROUND(COALESCE(r.attributed_revenue_usd, 0), 2) AS attributed_revenue_usd,
    ROUND((a.total_spend) / (NULLIF(r.attributed_orders, 0)), 2) AS cpa,
    ROUND((r.attributed_revenue_usd) / (NULLIF(a.total_spend, 0)), 2) AS roas
