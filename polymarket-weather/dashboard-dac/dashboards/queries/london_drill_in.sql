...
-- on the city's #1 primary-station anomaly day (London City EGLC, 2026-04-26).
WITH temps AS (
  SELECT
    ts_local,
  ...
  WHERE city = 'London'
    AND DATE(ts_local) = DATE '2026-04-26'
  GROUP BY ts_local
),
winning AS (
  SELECT m.market_id
  ...
  WHERE m.series_slug = 'london-daily-weather'
    ...
    AND m.end_local_date = DATE '2026-04-26'
...
prices_hourly AS (
  ...
  WHERE p.outcome_label = 'Yes'
    AND DATE(p.ts_utc, 'Europe/London') = DATE '2026-04-26'
